...
/**
 * Populate Store Items Script with Hugging Face Embeddings
 * 
 * This script populates the store_items table with 100+ realistic Walmart items
 * using Hugging Face's free sentence-transformers for embeddings.
 */
...
const HUGGINGFACE_API_KEY = process.env.HUGGINGFACE_API_KEY; // Optional, for higher rate limits
...
async function generateEmbedding(text) {
  try {
    // Use a different embedding model that works better with the API
    ...
    // Debug: log the response structure for first few items
    if (Math.random() < 0.1) { // Only log 10% of responses to avoid spam
      console.log('API Response structure:', typeof data, Array.isArray(data) ? data.length : 'not array');
    }
    ...
    if (Array.isArray(data)) {
      if (data.length > 0 && Array.isArray(data[0])) {
        ...
        return data; // Direct vector format
      }
    }
    
    // Handle object with embeddings property
    if (data && data.embeddings) {
      return data.embeddings;
    }
    
    // Handle direct vector in object
    if (data && Array.isArray(data) && data.every(item => typeof item === 'number')) {
      return data;
    }
...
async function populateStoreItems() {
  logInfo('Populating store items with Hugging Face embeddings...');
  ...
  for (let i = 0; i < storeItems.length; i++) {
    ...
    if (!embedding) {
      logWarning(`Skipping ${item.name} - failed to generate embedding`);
      errorCount++;
      continue;
    }
...
async function main() {
  try {
    console.log(`${colors.bold}${colors.blue}🛒 Populating Store Items Database (Hugging Face Embeddings)${colors.reset}`);
    console.log('============================================================');
    ...
    logSuccess(`Successfully added: ${successCount} items with embeddings`);
    if (errorCount > 0) {
      logError(`Failed to add: ${errorCount} items`);
    }
    
    logSuccess(`\n🎉 Store population complete! Added ${successCount} items with Hugging Face embeddings.`);
