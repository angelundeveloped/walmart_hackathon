...
/**
 * Populate Store Items Script
 * 
 * This script populates the store_items table with 100+ realistic Walmart items
 * including coordinates, categories, and embeddings for testing the RAG system.
 */
...
async function populateStoreItems() {
  ...
  for (let i = 0; i < storeItems.length; i++) {
    ...
    if (error) {
      ...
      logSuccess(`Added: ${item.name} (${item.category})`);
      successCount++;
    }
...
// Main function
async function main() {
  log('🛒 Populating Store Items Database', 'bold');
  log('=' .repeat(50), 'blue');
  ...
  try {
    ...
    logSuccess(`\n🎉 Store population complete! Added ${successCount} items with embeddings.`);
