...
async function testRAGSystem() {
  log.header('RAG System Test');
  
  try {
    // Test if RAG system can be imported and initialized
    const { ragSystem } = await import('../src/lib/rag.js');
    
    if (ragSystem) {
      recordTest(true, 'RAG system imported successfully');
      ...
      await ragSystem.initialize(testUserContext);
      recordTest(true, 'RAG system initialized successfully');
      
    } else {
      recordTest(false, 'RAG system import failed');
    }
    
  } catch (error) {
    recordTest(false, `RAG system test error: ${error.message}`);
  }
}
